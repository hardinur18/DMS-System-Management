...
set custom_cycle_days = 6
where payroll_method = 'custom'
  and custom_cycle_days is null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'employees_custom_cycle_days_range'
      and conrelid = 'public.employees'::regclass
  ) then
    alter table public.employees
      add constraint employees_custom_cycle_days_range
      check (custom_cycle_days is null or custom_cycle_days between 1 and 31);
  end if;
end;
$$;

comment on column public.employees.custom_cycle_days is
  'Target hari kerja valid untuk payroll_method custom. Null untuk metode non-custom.';
...
begin
  with employee_settings as (
    select
      ...
      case
        when employees.payroll_method = 'custom'
          then least(greatest(coalesce(employees.custom_cycle_days, 6), 1), 31)
        else 26
      end as target_days,
      case
  ...
  grouped_days as (
    select
      ...
      (
        ...
        + (((counted_days.opening_days + counted_days.attendance_index - 1) / counted_days.target_days) + 1)::integer
      ) as cycle_number,
      (((counted_days.opening_days + counted_days.attendance_index - 1) % counted_days.target_days) + 1)::integer as cycle_day_number
    from counted_days
  ),
  cycle_source as (
    select
      ...
      max(grouped_days.target_days)::integer as target_work_days,
      case
      ...
      case when max(grouped_days.cycle_day_number) >= max(grouped_days.target_days) then max(grouped_days.attendance_date) else null end as period_closed_at,
      least(max(grouped_days.cycle_day_number)::integer, max(grouped_days.target_days)::integer) as work_days_count
    from grouped_days
  ...
  upserted_cycles as (
    ...
    select
      ...
      case
        ...
        when employees.salary_type = 'monthly' then round((employees.monthly_salary / greatest(cycle_source.target_work_days, 1)) * cycle_source.work_days_count, 2)
      ...
      case when cycle_source.work_days_count >= cycle_source.target_work_days then 'ready' else 'active' end as status,
      case when cycle_source.work_days_count >= cycle_source.target_work_days then now() else null end as ready_at
    from cycle_source
    ...
    on conflict (employee_id, cycle_number) do update set
      ...
      target_work_days = case
        when public.payroll_cycles.status in ('locked', 'paid', 'void') then public.payroll_cycles.target_work_days
        else excluded.target_work_days
      end,
      salary_type = case
  ...
  from public.attendance_daily_summaries
  where attendance_daily_summaries.employee_id = target_employee_id
    and attendance_logs.employee_id = target_employee_id
