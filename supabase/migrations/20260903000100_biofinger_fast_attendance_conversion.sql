-- Classify Biofinger events from DMS shift windows, not from device punch rules.
-- The fingerprint machine is only the source of employee identity and timestamp.
...
set import_status = 'mapped',
    converted_attendance_log_id = null,
    notes = concat_ws(E'\n', nullif(events.notes, ''), 'SYSTEM: Dibuka ulang untuk klasifikasi berdasarkan shift DMS, bukan punch mesin.'),
    updated_at = now()
where events.import_status in ('converted', 'ignored', 'error')
  and events.employee_id in (
    select employees.id
    from public.employees
    where employees.employee_code in ('EMP-040', 'EMP-049', 'EMP-073', 'EMP-075')
  )
  and events.device_event_at >= timestamp with time zone '2026-09-02 00:00:00+07'
  and events.device_event_at < timestamp with time zone '2026-09-03 00:00:00+07';

delete from public.attendance_logs as logs
where logs.source = 'biofinger'
  and logs.employee_id in (
    select employees.id
    from public.employees
    where employees.employee_code in ('EMP-040', 'EMP-049', 'EMP-073', 'EMP-075')
  )
  and logs.attendance_date = date '2026-09-02';

select public.convert_biofinger_attendance_events(null, 5000);

select public.refresh_attendance_daily_summaries(date '2026-09-02', date '2026-09-02');

insert into public.audit_logs (actor_name, action, target_table, target_id, status, metadata)
values (
  'System',
  'Fast Biofinger attendance conversion',
  'biofinger_attendance_events',
  '20260903000100',
  'success',
  '{"source":"migration","module":"biofinger","policy":"dms-shift-window-fast-conversion"}'::jsonb
);
