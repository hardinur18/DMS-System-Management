-- Overtime full-duration policy.
-- Weekday overtime is paid only after shift obligation is fulfilled.
-- Sunday/holiday overtime can be paid from the full realized work duration.

alter table public.payroll_components
add column if not exists overtime_basis text not null default 'extra_after_shift';

alter table public.overtime_requests
add column if not exists overtime_basis text not null default 'extra_after_shift';

update public.payroll_components
set overtime_basis = case
  when auto_detect_overtime = true and day_type in ('sunday', 'holiday') then 'full_duration'
  else 'extra_after_shift'
end
where overtime_basis is null
   or auto_detect_overtime = true
   or code in ('OT-WEEKDAY', 'OT-SUNDAY');

update public.overtime_requests requests
set overtime_basis = coalesce(
  components.overtime_basis,
  case when requests.day_type in ('sunday', 'holiday') then 'full_duration' else 'extra_after_shift' end
)
from public.payroll_components components
where requests.payroll_component_id = components.id;

update public.overtime_requests
set overtime_basis = case
  when day_type in ('sunday', 'holiday') then 'full_duration'
  else 'extra_after_shift'
end
where payroll_component_id is null
   or overtime_basis is null;

do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'payroll_components_overtime_basis_check') then
    alter table public.payroll_components
      add constraint payroll_components_overtime_basis_check
      check (overtime_basis in ('extra_after_shift', 'full_duration'));
  end if;

  if not exists (select 1 from pg_constraint where conname = 'overtime_requests_overtime_basis_check') then
    alter table public.overtime_requests
      add constraint overtime_requests_overtime_basis_check
      check (overtime_basis in ('extra_after_shift', 'full_duration'));
  end if;
end $$;
...
update public.payroll_components
set
  description = 'Rate lembur per jam untuk hari kerja. Payable hanya setelah kewajiban shift terpenuhi.',
  overtime_basis = 'extra_after_shift',
  updated_at = now()
where code = 'OT-WEEKDAY';

update public.payroll_components
set
  description = 'Rate lembur per jam khusus hari Minggu/libur. Payable dihitung dari full durasi kerja aktual.',
  overtime_basis = 'full_duration',
  updated_at = now()
where code = 'OT-SUNDAY';
...
begin
  ...
  eligible_candidates as (
    select
      ...
      case
        ...
        else least(summary_candidates.post_shift_minutes, summary_candidates.extra_work_minutes)
  ...
  set
    ...
    notes = trim(both E'\n' from concat_ws(
      ...
      'Auto reset: tidak ada menit lembur payable setelah policy lembur dihitung ulang.'
    )),
    updated_at = now()
  ...
  select
    ...
    true,
    format(
      'Auto-detected dari settlement harian. Basis %s. Wajib %s menit, aktual %s menit, pulang lewat %s menit, lembur payable %s menit.',
      eligible.overtime_basis,
...
values
  ('System', 'Create overtime full-duration policy', 'overtime_requests', '20260827000300', 'success', '{"source":"migration","module":"overtime-payroll","policy":"weekday-extra-after-shift-sunday-full-duration"}'::jsonb);
