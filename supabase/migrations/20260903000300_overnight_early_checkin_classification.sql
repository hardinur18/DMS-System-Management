-- Treat early Biofinger punches before overnight shift start as check-in.
-- The fingerprint machine remains only the source of employee identity and timestamp.
...
begin
  ...
  with limited_events as (
    ...
    where events.import_status = 'mapped'
      ...
      and (
        events.normalized_event_type in ('check_in', 'check_out')
        or events.punch = 255
        or coalesce(events.normalized_event_type, '') = 'unknown'
      )
    ...
    select
      ...
      case
        ...
        when limited_events.same_start_time is not null
         ...
         and limited_events.local_event_time >= (limited_events.same_start_time - interval '4 hours')::time
      ...
      case
        ...
        when limited_events.same_start_time is not null
         and limited_events.same_end_time is not null
         and limited_events.local_event_time
             < limited_events.same_start_time + ((limited_events.same_end_time - limited_events.same_start_time) / 2)
          then 'check_in'
        when limited_events.same_start_time is not null
         and limited_events.same_end_time is not null
          then 'check_out'
  ...
  upserted_logs as (
    ...
    select
      ...
      concat(
        ...
        '; classified by DMS shift window.'
      )
  ...
  update public.biofinger_attendance_events as events
  set import_status = 'ignored',
      attendance_date = candidates.effective_attendance_date,
      normalized_event_type = candidates.effective_event_type,
      notes = concat_ws(E'\n', nullif(events.notes, ''), 'SYSTEM: Tidak dikonversi karena sudah ada attendance log manual untuk karyawan/tanggal kerja/event ini.'),
      updated_at = now()
  from tmp_biofinger_candidates as candidates
  join public.attendance_logs as existing_logs
    on existing_logs.employee_id = candidates.employee_id
  ...
  employees_refreshed := (select count(distinct employee_id)::integer from tmp_biofinger_upserted);
  return next;
end;
$$;
...
-- Repair current launch-day Biofinger rows that were classified by device punch.
update public.biofinger_attendance_events as events
set import_status = 'mapped',
    converted_attendance_log_id = null,
    notes = concat_ws(E'\n', nullif(events.notes, ''), 'SYSTEM: Dibuka ulang untuk klasifikasi early check-in shift malam.'),
    updated_at = now()
from (
  select candidate_events.id
  from public.biofinger_attendance_events candidate_events
  join public.employees employees on employees.id = candidate_events.employee_id
  join lateral public.resolve_employee_shift_for_date(
    employees.id,
    (candidate_events.device_event_at at time zone 'Asia/Jakarta')::date
  ) shift_row on true
  where candidate_events.import_status = 'ignored'
    and candidate_events.converted_attendance_log_id is null
    and shift_row.start_time is not null
    and shift_row.end_time is not null
    and shift_row.end_time <= shift_row.start_time
    and (candidate_events.device_event_at at time zone 'Asia/Jakarta')::time >= (shift_row.start_time - interval '4 hours')::time
    and (candidate_events.device_event_at at time zone 'Asia/Jakarta')::time < shift_row.start_time
) candidates
where events.id = candidates.id;
